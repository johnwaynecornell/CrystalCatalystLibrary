...
P_INSTANCE(PixInfo)  PixInfo::get(std::string pixformat) {
    P_INSTANCE(PixInfo) P = new PixInfo();

    P->pixformat = pixformat;

    size_t colon = pixformat.find(':');
    std::string _width;
    ...
    std::string _channel_type = pixformat.substr(colon + 1);
...
ReturnBuffer Pixels_ConvertPixels(utf8_string_const pixformat, utf8_string_const pixformat_dest, P_ELEMENTS(void)  pixdata, size_t pixdata_length, int32_t width, int32_t height) {
    ReturnBuffer Ret = {};
    ...
    Ret.element_count = pix_count;
    Ret.element_size = dst_pixinfo->pix_stride;
    Ret.memory = new uint8_t[pix_count * dst_pixinfo->pix_stride];

    Ret.UnderDeallocate_elements = ReturnBuffer_NonDeallocate;
    Ret.UnderDeallocate_memory = ReturnBuffer_Delete_array<uint8_t>;

    int8_t *src_ptr = (int8_t *)pixdata;
    int8_t *dst_ptr = (int8_t *)Ret.memory;
