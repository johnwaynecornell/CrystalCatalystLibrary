...
void DataInterchange_Free(P_INSTANCE(DataInterchange) drag) {
 ...
 bool DataInterchange_FormatExists(P_INSTANCE(DataInterchange) data, utf8_string_const format) {
    std::string f = format;
...
P_INSTANCE(DataInterchange::Node)  DataInterchange_FormatAdd(P_INSTANCE(DataInterchange) drop, utf8_string_const format) {
...
void DataInterchange_FormatEnum_Text(P_INSTANCE(DataInterchange::Node) node, P_OUT(utf8_string_const) text) {
    *text = node->type;
}
...
void DataInterchange_Selection_Reveal(P_INSTANCE(DataInterchange) drag, P_OUT(utf8_string_const) format, P_OUT(P_INSTANCE(void)) data, P_OUT(size_t) size)
{
...
void DataInterchange_Selection_Set(P_INSTANCE(DataInterchange) drag, utf8_string_const format, P_INSTANCE(void) data, size_t size) {
