...
void on_mouse_down(P_INSTANCE(WindowHandle) window_handle, int32_t button, int32_t x, int32_t y) {
    ...
    if (button == 1) {
        ...
        if (format != nullptr)  DataInterchange_Select(data, format);
...
void receive(std::string label, P_INSTANCE(WindowHandle) window_handle, P_INSTANCE(DataInterchange)  data) {
    ...
    if (StartingWith("text/", type)) {
       ...
       CommandSpawn *bash = CommandSpawn_Create();
       CommandSpawn_SetShell(bash, CommandSpawn_GetShell_Bash(bash));

       CommandSpawn_Command(bash, "SimplifyHTML -stdin -GPT -header-off -condense -div-off -verbose-tables", (char *)data_ptr, CommandSpawn::E_PIPE_STDOUT);

       payload = CommandSpawn_ReadToEnd(bash, CommandSpawn::E_PIPE_STDOUT);

        CommandSpawn_Join(bash);
        CommandSpawn_Destroy(bash);
...
int32_t main(int32_t argc, P_ELEMENTS(char *)  argv) {
    ...
    P_INSTANCE(WindowHandle) window_handle = CrystalWindow_CreateSimple(32, 32, "htmlify Window");
